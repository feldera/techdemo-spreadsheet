...
use egui::{Color32, Key, OpenUrl, Pos2, Rect, RichText, Sense, Vec2};
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.with_layout(egui::Layout::left_to_right(egui::Align::TOP), |ui| {
                            ...
                            ui.horizontal(|ui| {
                                for _ in 0..stats.currently_active_users.min(10) {
                                    ui.painter().circle_filled(
                                        Pos2::new(ui.cursor().min.x + icon_size.x, ui.cursor().center().y),
                                        icon_size.x / 2.0,
                                        Color32::LIGHT_GREEN,
                                    );
                                    ui.add_space(12.0);
                                ...
                                if stats.currently_active_users > 10 {
                                    ui.label(format!("+{} more", stats.currently_active_users - 10));
                                }
                            });
                        });
                        ...
                        ui.vertical(|ui| {
                            ...
                            let max_cells = (SpreadsheetApp::DEFAULT_COLS as u64 * SpreadsheetApp::DEFAULT_ROWS as u64) as f64;
                            ...
                            ui.painter().rect_filled(
                                Rect::from_min_size(ui.cursor().min, Vec2::new(filled_ratio * 150.0, 15.0)),
                                4.0,
                                filled_color,
                            );
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::TOP).with_main_wrap(true), |ui| {
                        ui.vertical(|ui| {
                            ...
                            ui.horizontal(|ui| {
                                ui.add(egui::Hyperlink::from_label_and_url(format!("{GITHUB} feldera"), "https://github.com/feldera/feldera").open_in_new_tab(true));
                                ui.add_space(10.0);
                                ui.add(egui::Hyperlink::from_label_and_url(format!("{GITHUB} axum"), "https://github.com/tokio-rs/axum").open_in_new_tab(true));
                                ui.add_space(10.0);
                                ui.add(egui::Hyperlink::from_label_and_url(format!("{GITHUB} egui"), "https://github.com/emilk/egui").open_in_new_tab(true));
                                ui.add_space(10.0);
                                ui.add(egui::Hyperlink::from_label_and_url(format!("{GITHUB} XLFormula Engine"), "https://github.com/jiradaherbst/XLFormula-Engine").open_in_new_tab(true));
                                ui.add_space(10.0);
                            });
                        });
                    });
            ...
            ui.vertical(|ui| {
                ...
                let color_response = egui::widgets::color_picker::color_edit_button_srgba(ui, &mut self.bg_color_picked, Alpha::OnlyBlend);
            ...
            TableBuilder::new(ui)
                ...
                .columns(Column::remainder(), self.num_cols)
                ...
                .header(Self::DEFAULT_ROW_HEIGHT+3.0, |mut header| {
                    let col_idx_to_label = |idx: usize| {
                        if idx < 26 {
                            ...
                            format!("{}{}", (b'A' + (idx / 26 - 1) as u8) as char, (b'A' + (idx % 26) as u8) as char)
                        }
                    };
                ...
                .body(|body| {
                    body.rows(Self::DEFAULT_ROW_HEIGHT, self.num_rows + 1, |mut row| {
                        ...
                        for col_index in 0..self.num_cols {
                            ...
                            let cell = self.cell_cache.get(id);
                            row.col(|ui| {
                                let has_focus = row_index == self.focused_row && col_index == self.focused_col;
                                let rect = ui.available_rect_before_wrap();
                                let resp = ui.interact(ui.available_rect_before_wrap(), ui.make_persistent_id(id), Sense::click());
                                ...
                                if has_focus {
                                    ui.painter().rect_stroke(rect, 0.0, egui::Stroke::new(1.0, Color32::LIGHT_BLUE));
                                }
                                        ...
                                        if let egui::Event::Key { key, pressed, .. } = i {
                                            if now - self.last_key_time > KEY_DELAY && *pressed {
                                                match key {
                                                    ...
                                                    Key::ArrowDown => if self.editing_cell.is_none() {
                                                        self.focused_row = (self.focused_row + 1).min(self.num_rows - 1);
                                                        self.last_key_time = now;
                                                    }
                                                    Key::ArrowUp => if self.editing_cell.is_none() {
                                                        self.focused_row = self.focused_row.saturating_sub(1);
                                                        self.last_key_time = now;
                                                    }
                                                    Key::ArrowRight => if self.editing_cell.is_none() {
                                                        self.focused_col = (self.focused_col + 1).min(self.num_cols - 1);
                                                        self.last_key_time = now;
                                                    }
                                                    Key::ArrowLeft => if self.editing_cell.is_none() {
                                                        self.focused_col = self.focused_col.saturating_sub(1);
                                                        self.last_key_time = now;
                                                    }
                                                    Key::PageDown => if self.editing_cell.is_none() {
                                                        self.focused_row = (self.focused_row + 10).min(self.num_rows - 1);
                                                        self.last_key_time = now;
                                                    }
                                                    Key::PageUp => if self.editing_cell.is_none() {
                                ...
                                if resp.clicked() || (cell_response.clicked() && !cell_response.has_focus()) {
                                ...
                                if self.editing_cell.is_none() && (resp.double_clicked() || cell_response.double_clicked() || (resp.has_focus() && ui.input(|i| i.key_pressed(Key::Enter)))) {
